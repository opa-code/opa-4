...
uses Vgraph, Graphics, OPAElements, OPAglobal, OPAtune, ASaux, mathLib, grids, sysutils, stdctrls, forms;
...
begin
  with Ella[j] do begin
    ...
    if err then opaLog(2,'OpticPlot>Ella_val expression evaluation error for '+ nam);
  end;
end;
...
begin
  ...
  fail:=false;
  R1:=MatUni4;
  opaLog(-2, 'OpticPlot>PropPer stacksize '+inttostr(Length(Tmatstack))+' '+inttostr(Length(Elenstack)));
